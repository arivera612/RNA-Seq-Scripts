...
# Current format : Rscript venn.R label1 file1 label2 file2 out_prefix
...
merge_raw <- merge(x=f1, y=f2, by.x ="V1", by.y="V2", sort=F)
print(merge_raw)
...
heat <- cbind(FC1, FC2, FC3, FH1, FH2, FH3, FH4, FH5)

rownames(heat) <- merge_sort$V1
#heat
heat
library(lattice)
...
levelplot(t(y), height=0.3, col.regions=colorpanel(40, "dimgrey","white","red"), main="", colorkey=list(space="top"), xlab="", ylab="", pretty=TRUE, width=0.5, cexRow=0.1, cexCol=0.1, aspect=2.5)
